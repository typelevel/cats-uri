...
// Projects
lazy val `cats-uri` = project.in(file("."))
...
lazy val site = project.in(file("site"))
  ...
  .settings{
    import microsites._
    Seq(
      micrositeName := "cats-uri",
      micrositeDescription := "A URI with Cats instances",
      micrositeAuthor := "Ross A. Baker",
      micrositeGithubOwner := "rossabaker",
      micrositeGithubRepo := "cats-uri",
      micrositeBaseUrl := "/cats-uri",
      micrositeDocumentationUrl := "https://www.javadoc.io/doc/org.typelevel/cats-uri_2.13",
...
inThisBuild(List(
  ...
  homepage := Some(url("https://github.com/rossabaker/cats-uri")),
  ...
  scalacOptions in (Compile, doc) ++= Seq(
      ...
      "-doc-source-url", "https://github.com/rossabaker/cats-uri/blob/v" + version.value + "€{FILE_PATH}.scala"
  )
))